...
# HTTPS is on by default via a self-signed cert (generated by
# gunicorn.conf.py before workers start — see tls_config.py). The Settings
# page manages the certificate at runtime; HTTPS=true covers the
# behind-a-TLS-terminating-proxy case.
import tls_config  # noqa: E402 — needs load_dotenv() above

_tls_cert_path, _tls_key_path = tls_config.resolve_tls(auto_generate=False)
TLS_ENABLED = bool(_tls_cert_path)
ENV_TLS = bool(os.environ.get('TLS_CERT') and os.environ.get('TLS_KEY'))
PROXY_TLS = os.environ.get('HTTPS', '').lower() == 'true'
app.config['SESSION_COOKIE_SECURE'] = TLS_ENABLED or PROXY_TLS
...
def _reload_server() -> bool:
    """Gracefully reload gunicorn (re-reads TLS config) after the response
    goes out. Returns False when not under gunicorn (dev server)."""
    if not _running_under_gunicorn():
        return False
    master = os.getppid()
    threading.Timer(1.5, lambda: os.kill(master, signal.SIGHUP)).start()
    return True


def _cert_info(path):
    """Human-readable subject/expiry summary of a PEM certificate."""
    try:
        out = subprocess.run(
        ...
        return {
            'subject': subject,
            'expires': fields.get('notAfter', '').strip(),
            'self_signed': fields.get('issuer', '').strip() == subject,
        }
    except Exception:
        return None
...
def _tls_state():
    cert, _ = tls_config.resolve_tls(auto_generate=False)
    return {
        'enabled': bool(cert),
        'env_managed': ENV_TLS,
        'proxy_mode': PROXY_TLS,
        'info': _cert_info(cert) if cert else None,
    }


def _flash_reload_result():
    if _reload_server():
        flash('The server is reloading — give it a few seconds, then reload '
              'this page (mind the http/https in the address bar).', 'info')
    else:
        flash('Restart the app to apply the certificate change.', 'warning')


@app.route('/admin/settings/tls/upload', methods=['POST'])
@login_required
def tls_upload():
    _check_csrf()
    if ENV_TLS:
        flash('The certificate is managed via TLS_CERT/TLS_KEY in .env', 'warning')
        return redirect(url_for('admin_settings'))
    cert = request.files.get('cert')
    key = request.files.get('key')
    if not cert or not cert.filename or not key or not key.filename:
        flash('Choose both a certificate file and a private key file', 'warning')
        return redirect(url_for('admin_settings'))
    os.makedirs(tls_config.tls_dir(), exist_ok=True)
    new_cert = tls_config.cert_file() + '.new'
    new_key = tls_config.key_file() + '.new'
    cert.save(new_cert)
    key.save(new_key)
    os.chmod(new_key, 0o600)
    try:
        # Rejects garbage and cert/key mismatches before anything goes live
        ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        ctx.load_cert_chain(new_cert, new_key)
    except Exception as e:
        os.unlink(new_cert)
        os.unlink(new_key)
        flash(f'Not a valid certificate/key pair: {e}', 'danger')
        return redirect(url_for('admin_settings'))
    os.replace(new_cert, tls_config.cert_file())
    os.replace(new_key, tls_config.key_file())
    if os.path.exists(tls_config.disabled_marker()):
        os.unlink(tls_config.disabled_marker())
    flash('Certificate installed — HTTPS will use it from the next reload', 'success')
    _flash_reload_result()
    return redirect(url_for('admin_settings'))


@app.route('/admin/settings/tls/self-signed', methods=['POST'])
@login_required
def tls_self_signed():
    _check_csrf()
    if ENV_TLS:
    ...
    if os.path.exists(tls_config.disabled_marker()):
        os.unlink(tls_config.disabled_marker())
    flash('New self-signed certificate generated (your browser will warn '
          'once — that is expected for self-signed certificates)', 'success')
    _flash_reload_result()
    return redirect(url_for('admin_settings'))


@app.route('/admin/settings/tls/disable', methods=['POST'])
@login_required
def tls_disable():
    _check_csrf()
    if ENV_TLS:
        ...
        return redirect(url_for('admin_settings'))
    os.makedirs(tls_config.tls_dir(), exist_ok=True)
    open(tls_config.disabled_marker(), 'w').close()
    flash('HTTPS disabled — switch your address bar to http:// after the '
          'reload. Note: camera barcode scanning will not work over HTTP.',
          'warning')
    _flash_reload_result()
    return redirect(url_for('admin_settings'))
